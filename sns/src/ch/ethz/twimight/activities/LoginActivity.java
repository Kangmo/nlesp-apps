...
/**
 * Logging the user in and out. Different things can happen, depending on
 * whether we have (i) tokens and/or (ii) connectivity: Tokens, Connectivity:
 * Start the Timeline. In the background verify the tokens and report only on
 * error. Tokens, no Connectivity: Start the Timeline. Display Toast about lack
 * of connectivity. No tokens: whether or not we have connectivity, we show the
 * login button. TODO: Dump the state in a file upon logout and read it again
 * when logging in.
 * 
 * @author thossmann
 * 
...
public class LoginActivity extends Activity implements OnClickListener {
	...
	// VICDATA add for sign in
	public static final String VICDATA_LOGIN_FAILED = "vicdata_login_failed";
	// END
	/** Name of Twitter ID in shared preferences */
	// VICDATA private -> public
	public static final String TWITTER_SCREENNAME = "twitter_screenname";
	// END
	/** Name of Twitter screenname in shared preferences */

	// VICDATA private -> public
	public static final String TWITTER_ACCESS_TOKEN = "twitter_access_token";
	/** Name of access token in preference */
	public static final String TWITTER_ACCESS_TOKEN_SECRET = "twitter_access_token_secret";
	// END
	/** Name of secret in preferences */

	private static final String TWITTER_REQUEST_TOKEN = "twitter_request_token";
	/** Name of the request token in preferences */
	private static final String TWITTER_REQUEST_TOKEN_SECRET = "twitter_request_token_secret";
	/** Name of the request token secret in preferences */
	...
	public static final String SIGNUP_RESULT = "vicdata_signup_result";
	...
	// VICDATA new initialization logic.
	private void launchSignInActivity(Context context) {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// these access token is dummy. See the Util.putAccessToken() method.
		String cachedLoginId = SignInActivity.getCachedLoginId(getBaseContext());
		setAccessToken(cachedLoginId + ":temp_key", this);
		setAccessTokenSecret("access_token_secret", this);
		
		// do sign up
		if (SignUpActivity.signiningUp) {
			Intent i = new Intent(TwitterService.SYNCH_ACTION);
			i.putExtra("synch_request", TwitterService.SYNCH_SIGNUP);
			
			progressDialog = ProgressDialog.show(LoginActivity.this,
					getString(R.string.title_in_progress), getString(R.string.info_creating_new_account));
		...
		if (hasAccessToken(this) && hasAccessTokenSecret(this)
			...
			if (cm.getActiveNetworkInfo() == null
				...
				Toast.makeText(this,
						getString(R.string.error_internet_not_connected),
			...
			progressDialog = ProgressDialog.show(LoginActivity.this,
					getString(R.string.title_in_progress), getString(R.string.info_verifying_your_account));
...
//		setAccessTokenSecret("access_token_secret", this);
//		// setTwitterId("VICDATA", this);
//		// setTwitterScreenname("VICDATA", this);
//		// END
//
...
//					|| !cm.getActiveNetworkInfo().isConnected()) {
//				Toast.makeText(this,
//						"Not connected to the Internet, showing old Tweets!",
//						Toast.LENGTH_LONG).show();
//			}
//			startTimeline(this);
//
...
//		}
		...
		//
		// } else {
		// // if we don't have request token and secret, we show the login
		// button
	...
	private void registerLoginReceiver() {
		...
		IntentFilter intentFilter = new IntentFilter(
				LoginActivity.LOGIN_RESULT_ACTION);
	...
	public void onClick(View view) {
		...
		case R.id.buttonLogin:
			...
			if (cm.getActiveNetworkInfo() != null
				...
				Toast.makeText(
						this,
						"Not connected to the Internet, please try again later!",
						Toast.LENGTH_LONG).show();
			}
			break;
		}
	...
	private class GetRequestTokenTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			OAuthConsumer consumer = new CommonsHttpOAuthConsumer(
					Obfuscator.getKey(), Obfuscator.getSecret());
	...
	private class GetAccessTokensTask extends AsyncTask<Uri, Void, String> {
		...
		protected void onPreExecute() {
			...
			super.onPreExecute();
			progressDialog = ProgressDialog.show(LoginActivity.this,
					"In progress", "Veryfing credentials");
		}
		...
		protected void onPostExecute(String result) {
			...
			// As a last step, we verify the correctness of the credentials and
			// retrieve our Twitter ID
			if (success) {
	...
	public static void startAlarms(Context context) {
		...
		if (PreferenceManager.getDefaultSharedPreferences(context).getBoolean(
				"prefTDSCommunication", Constants.TDS_DEFAULT_ON) == true) {
		...
		if (PreferenceManager.getDefaultSharedPreferences(context).getBoolean(
				"prefRunAtBoot", Constants.TWEET_DEFAULT_RUN_AT_BOOT) == true) {
...
//			dbHelper.flushDB();
			...
			SearchRecentSuggestions suggestions = new SearchRecentSuggestions(
					context, TwimightSuggestionProvider.AUTHORITY,
					TwimightSuggestionProvider.MODE);
			suggestions.clearHistory();
			
			// VICDATA additional logout actions.
			SignInActivity.clearCachedLoginInfo(context);
			// END
	...
	/**
	 * True if we have an access token in the shared preferences, false
	 * otherwise
	 * 
	 * @param context
	 * @return
	...
	/**
	 * True if we have a request token in the shared preferences, false
	 * otherwise
	 * 
	 * @param context
	 * @return
	...
	/**
	 * True if we have a request token secret in the shared preferences, false
	 * otherwise
	 * 
	 * @param context
	 * @return
	...
	// VICDATA
	public static void removeTwitterId(Context context) {
		SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(context);
		SharedPreferences.Editor prefEditor = prefs.edit();
		prefEditor.remove(TWITTER_ID);
		prefEditor.commit();
	}
	// END
				...
				if (intent.getAction() != null) {
					...
					if (intent.getAction().equals(
						...
						if (intent.hasExtra(LoginActivity.LOGIN_RESULT)) {
							...
							if (intent.getIntExtra(LoginActivity.LOGIN_RESULT,
									LoginActivity.LOGIN_FAILURE) == LoginActivity.LOGIN_SUCCESS) {
								startTimeline(context);
							} else {
								// VICDATA show sign in UI again.
								VdUtil.showLastError(LoginActivity.this);
								launchSignInActivity(context);
								// message will be displayed on SignIn UI.
//								Toast.makeText(
//										getBaseContext(),
//										"There was a problem with the login. Please try again later.",
//										Toast.LENGTH_SHORT).show();
								// END
							}
						}
						// VICDATA handling sign up result.
						if (intent.hasExtra(LoginActivity.SIGNUP_RESULT)) {
							SignUpActivity.signiningUp = false;
							if (progressDialog != null) {
								progressDialog.dismiss();
							}
							if (intent.getIntExtra(LoginActivity.SIGNUP_RESULT,
									LoginActivity.SIGNUP_FAILURE) == LoginActivity.SIGNUP_SUCCESS) {
								String cachedLoginId = SignInActivity.getCachedLoginId(context);
								setAccessToken(cachedLoginId + ":temp_key", context);
								setAccessTokenSecret("access_token_secret", context);
