...
public class ShowTweetActivity extends TwimightBaseActivity{	
	...
	private TextView textRetweeted_by;
	...
	private void setupButtons() {
		...
		// VICDATA deletion is not supported yet
		...
		if(c.getLong(c.getColumnIndex(Tweets.COL_TID)) != 0 || 
			...
			replyButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// VICDATA add tweet_owner_id.
					Log.d(TAG, "tweet_owner_id = " + c.getLong(c.getColumnIndex(Tweets.COL_USER)));
					i.putExtra("tweet_owner_id", c.getLong(c.getColumnIndex(Tweets.COL_USER))); // reply to user id
					i.putExtra("original_tweet_id", c.getLong(c.getColumnIndex(Tweets.COL_REPLYTO))); // reply to tweet id
					// END
					// VICDATA not used 
//					i.putExtra("text", "@"+c.getString(c.getColumnIndex(TwitterUsers.COL_SCREENNAME))+ " ");
	...
	private void showReplyButton() {
		final long tweetId = c.getLong(c.getColumnIndex(Tweets.COL_TID));
		final long tweetOwnerId = c.getLong(c.getColumnIndex(Tweets.COL_USER));
		final long originalTweetId = c.getLong(c.getColumnIndex(Tweets.COL_REPLYTO));
		
		Log.d(TAG, "cur_tweet_id = " + tweetId);
		Log.d(TAG, "tweet_owner_id = " + tweetOwnerId);
		Log.d(TAG, "original_tweet_id = " + originalTweetId);
		
		if (tweetId == 0L) {
			Log.d(TAG, "Tweet ID is 0. Ignore it.");
			return;
		}
				
		replyButton.setVisibility(LinearLayout.VISIBLE);
		replyButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent i = new Intent(getBaseContext(), NewTweetActivity.class);
				i.putExtra("isReplyTo", tweetId);
				i.putExtra("tweet_owner_id", tweetOwnerId); // reply to user id
				i.putExtra("original_tweet_id", originalTweetId); // reply to tweet id
				startActivity(i);
			}
		});
//		}
	...
	private void setProfilePicture() {
		...
		if(!c.isNull(c.getColumnIndex(TwitterUsers.COL_PROFILEIMAGE))){
			...
			Log.d(TAG, "VICDATA: has ProfilePicture = " + (imageByteArray != null));
			if (imageByteArray != null) {				
				Log.d(TAG, "VICDATA: size of ProfilePicture = " + imageByteArray.length);
...
//		screenNameView.setText("@"+screenName);
	...
	private void refreshCommentCount() {
		// VICDATA show number of comments instead of 'retweeted_by'.
		Long commentCount = c.getLong(c.getColumnIndex(Tweets.COL_RETWEETCOUNT));
		textRetweeted_by = (TextView) findViewById(R.id.showTweetRetweeted_by);
		if (commentCount != null) {
			if (commentCount > 0L) {
				textRetweeted_by.setText(R.string.label_num_of_comments);
				textRetweeted_by.append(Long.toString(commentCount));
				textRetweeted_by.setVisibility(View.VISIBLE);	
				
				final Long tweetOwnerId = c.getLong(c.getColumnIndex(Tweets.COL_USER));
				final Long tweetId = c.getLong(c.getColumnIndex(Tweets.COL_REPLYTO));
				Log.d(TAG, "tweetOwnerId: " + tweetOwnerId);
				Log.d(TAG, "tweetId: " + tweetId);
				textRetweeted_by.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View view) {
						Intent i = new Intent(getBaseContext(), ShowCommentListActivity.class);
...
//		}					
	...
	public void onDestroy(){
		...
		if (textRetweeted_by != null) {
			textRetweeted_by.setOnClickListener(null);
		}
		c.close();
