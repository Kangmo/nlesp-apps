...
public class OAuthScribeClient implements IHttpClient {
	...
	public String getPage(String uri, Map<String, String> vars,
		...
		// AccessToken을 shared preference에 저장
		...
		try {
			...
			// VKIT log get request
			Log.d("VKIT", "[OAuthScribeClient::getPage] URI: " + uri);
			Log.d("VKIT", "[OAuthScribeClient::getPage] authenticate: "
					+ authenticate);
			// END
	...
	public String post(String uri, Map<String, String> vars,
		...
		try {
			...
			assert canAuthenticate();
			// VKIT log request
			Log.d("VKIT", "[OAuthScribeClient::post] URI: " + uri);
			// END
			...
			if (vars != null && vars.size() != 0) {
				for (Entry<String, String> e : vars.entrySet()) {
					...
					// VKIT log request
					Log.d("VKIT",
							"[OAuthScribeClient::post] var: " + e.getKey()
									+ " = " + e.getValue());
					// END
